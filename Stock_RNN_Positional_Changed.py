...
class RNNPositional(nn.Module):
    def __init__(self, input_dim, h_dim, embed_dim, bt_size, seq_length, n_layers, out_dim, bidirectional, dropout):
...
class TimeEmbedding(nn.Module):
    def __init__(self, d_embed):
        ...
        ## concat にするか sum　にするか -> sumだとPriceの情報が失われてしまう
...
class DirectionalMSELoss(nn.Module):
    def __init__(self, loss_func, outputs_size, mse_scaler, scale):
    ...
    def forward(self, inputs, label, outputs):
        ...
        true_dif = inputs[:, -1, :self.outputs_size] - label[:, :] 
        pred_dif = inputs[:, -1, :self.outputs_size] - outputs[:, :]
...
class PositionalEmbedding(nn.Module):
    def __init__(self, seq_length, bt_size):
        ...
        lookup = torch.LongTensor([i for i in range(seq_length)])
        self.lookup = torch.stack([lookup for _ in range(bt_size)], dim=0)
...
def get_model_2_(input_size, outputs_size, embed_dim, bt_size, seq_length, hidden_dim, n_layers, bidirectional, dropout, lr, scheduler_step, gamma, mse_scaler, log_dir, scale):
    
    model = RNNPositional(input_size, hidden_dim, embed_dim, bt_size, seq_length, n_layers, outputs_size, bidirectional, dropout)
    optimizer = optim.Adam(model.parameters(), lr=lr)
    mse_loss = nn.MSELoss()
    total_loss_func = DirectionalMSELoss(mse_loss, outputs_size, mse_scaler, scale)
